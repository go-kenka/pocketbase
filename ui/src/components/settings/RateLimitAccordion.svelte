...
<OverlayPanel bind:this={formatInfoPanel}>
    <svelte:fragment slot="header">
        <h4 class="center txt-break">限制标签格式</h4>
    </svelte:fragment>

    <p>限制规则按以下顺序解析（遇到第一个匹配项时停止）：</p>
    <ol>
        <li>精确标签（例如 <code>users:create</code>）</li>
        <li>通配符标签（例如 <code>*:create</code>）</li>
        <li>请求方法 + 精确路径（例如 <code>POST /a/b</code>）</li>
        <li>请求方法 + 前缀路径（例如 <code>POST /a/b<strong>/</strong></code>）</li>
        <li>精确路径（例如 <code>/a/b</code>）</li>
        <li>前缀路径（例如 <code>/a/b<strong>/</strong></code>）</li>
    </ol>
    <p>
        如果存在多个具有相同标签但目标用户不同的规则（例如"游客"与"已认证用户"），
        只会考虑匹配的目标用户规则。
    </p>
    ...
    <ul>
        <li class="m-b-sm">
            <code>[METHOD ]/my/path</code> - 完整的精确路由匹配（
            <strong>不能带有尾部斜杠</strong>；"请求方法"是可选的）。
            <br /> 例如：
            <ul class="m-0">
                <li class="m-0">
        ...
        <li class="m-b-sm">
            <code>[METHOD ]/my/prefix<strong>/</strong></code> - 路径前缀（
            <strong>必须以斜杠结尾；</strong>
            "请求方法"是可选的）。例如：
            <ul class="m-0">
                <li class="m-0">
        ...
        <li>
            <code>collectionName:predefinedTag</code> - 针对单个集合的特定操作。要
            对所有集合应用规则，可以使用 <code>*</code> 通配符。例如：
            <code>posts:create</code>、<code>users:listAuthMethods</code>、<code>*:auth</code>。
            <br />
            预定义的集合标签有（<em>输入时会有自动补全提示</em>）：
            <ul>
