...
<Accordion single>
    ...
    <div class="content m-b-sm">
        <p>
            当PocketBase部署在Fly等平台上或通过NGINX等代理访问时，来自不同用户的请求将源自同一个IP地址（连接到您的PocketBase应用的代理IP）。
        </p>
        <p>
            在这种情况下，为了获取实际用户IP（用于速率限制、日志记录等），您需要正确配置代理并在下方列出PocketBase可用于提取用户IP的可信头部。
        </p>
        <p class="txt-bold">使用此类代理时，为避免欺骗，建议：</p>
        <ul class="m-t-0 txt-bold">
            <li>使用仅由代理控制且用户无法手动设置的头部</li>
            <li>确保PocketBase服务器只能通过代理访问</li>
        </ul>
        <p>如果PocketBase未部署在代理后面，您可以清除头部字段。</p>
    </div>

    <div class="grid grid-sm">
        <div class="col-lg-9">
            <Field class="form-field m-b-0" name="trustedProxy.headers" let:uniqueId>
                <label for={uniqueId}>可信代理头部</label>
                <MultipleValueInput
                    id={uniqueId}
                    placeholder="留空以禁用"
            ...
            <Field class="form-field m-0" name="trustedProxy.useLeftmostIP" let:uniqueId>
                <label for={uniqueId}>
                    ...
                    <i
                        ...
                        use:tooltip={{
                            text: "当代理返回多个IP作为头部值时使用。最右侧的IP通常被认为是最可信的，但这可能因代理而异。",
                            position: "right",
                        }}
                    />
                </label>
