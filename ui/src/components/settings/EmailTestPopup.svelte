...
>
    ...
    <form id={formId} autocomplete="off" on:submit|preventDefault={() => submit()}>
        ...
        {#if showAuthCollections}
            <Field class="form-field required" name="collection" let:uniqueId>
                ...
                <ObjectSelect
                    id={uniqueId}
                    selectPlaceholder={isAuthCollectionsLoading ? "正在加载认证集合..." : "选择认证集合"}
        ...
        <Field class="form-field required m-0" name="email" let:uniqueId>
            <label for={uniqueId}>收件邮箱地址</label>
    ...
    <svelte:fragment slot="footer">
        <button type="button" class="btn btn-transparent" on:click={hide} disabled={isSubmitting}>关闭</button
        >
        <button
            type="submit"
            form={formId}
